...
##_##COUNT_ID <- 'syn23593968';
##_##ALL_USED_IDs <- COUNT_ID
##_##COUNT_OBJ <- syn_temp$get(COUNT_ID, version = 1)
#COUNT <- read.table(COUNT_OBJ$path, header=T, sep='\t', check.names = F, row.names = 1)
##_##COUNT <- as.data.frame(data.table::fread(COUNT_OBJ$path, header=T, sep='\t', check.names = F))#, row.names = 1)
##_##row.names(COUNT) <- as.character(COUNT$feature)
##_##COUNT <- COUNT[ ,(colnames(COUNT) %in% 'feature')==F ]
##_##COUNT <- as.matrix(COUNT)

#Toss Zero variance genes post residual correct
##_##COUNT <- COUNT[ (row.names(COUNT) %in% c("ENSG00000239467", "ENSG00000005486", "ENSG00000160972", "ENSG00000165886", "ENSG00000002330", "ENSG00000149823", "ENSG00000139722", "ENSG00000169217", "ENSG00000127774", "ENSG00000122490", "ENSG00000130881", "ENSG00000126005", "ENSG00000171222", "ENSG00000070413", "ENSG00000211459")) == F, ]

##_##Provenance <- 'syn21075913'
##_##Prov <- syn_temp$get(Provenance, version = 1)
##_##ALL_USED_IDs[length(ALL_USED_IDs)+1] = Provenance
##_##Translator <- read.csv(Prov$path, header=T, row.names = 1)
...
##_##COUNT <- COUNT[ rowSums(COUNT[,] > 0) / (ncol(COUNT)) >= 0.05, ]

# Convert rownames of counts from tracking id to ensemble gene id
##_##tmp = data.frame(Gene.ID = rownames(COUNT)) %>%
##_##  dplyr::mutate(ID = Gene.ID) %>%
##_##  tidyr::separate(ID, c('ensembl_gene_id', 'position'), sep = '\\.')
##_##rownames(tmp) = tmp$Gene.ID
##_##rownames(COUNT) = tmp[rownames(COUNT), 'ensembl_gene_id']

# Download expression data
##_##Meta_ID <- 'syn23573928';
##_##ALL_USED_IDs <- c( Meta_ID, ALL_USED_IDs )
##_##Meta_OBJ <- syn_temp$get( Meta_ID, version = 2 )
##_##Meta <- read.csv( Meta_OBJ$path, header=T, check.names = F, row.names = 1 )

#Filter for Rin Here#
##_##Meta <- Meta[ Meta$RIN >= 5 | Meta$sampleid %in% c('380_120503','500_120515'), ]

#Filter Out Sample Swaps
##_##Meta <- Meta[ Meta$FailQC_SampleSwapped == 0, ]
##_##COUNT <- COUNT[ ,row.names(Meta) ]
...
COUNT_ID <- 'syn23643275';
ALL_USED_IDs <- COUNT_ID
COUNT_OBJ <- syn_temp$get(COUNT_ID, version = 1)
COUNT <- load(COUNT_OBJ$path)

COUNT <- cqn_counts$counts

CQN.GENE_EXPRESSION <- cqn_counts
# Download expression data
Meta_ID <- 'syn23643273';
ALL_USED_IDs <- c( Meta_ID, ALL_USED_IDs )
Meta_OBJ <- syn_temp$get( Meta_ID, version = 1 )
Meta <- load( Meta_OBJ$path )

clean_md$diagnosis <- as.character('other')
clean_md$diagnosis[ as.numeric(as.character(clean_md$cogdx)) == 1 & 
                    as.numeric(as.character(clean_md$braaksc)) <= 3 & 
                    as.numeric(as.character(clean_md$ceradsc)) >= 3] <- as.character('control')


clean_md$diagnosis[as.numeric(as.character(clean_md$cogdx)) == 4&
                   as.numeric(as.character(clean_md$braaksc)) >= 4 & 
                   as.numeric(as.character(clean_md$ceradsc)) <= 2] = as.character('AD')
clean_md$diagnosis <- as.factor(clean_md$diagnosis)

import_metadata$diagnosis <- as.character('other')
import_metadata$diagnosis[ as.numeric(as.character(import_metadata$cogdx)) == 1 & 
                           as.numeric(as.character(import_metadata$braaksc)) <= 3 &
                           as.numeric(as.character(import_metadata$ceradsc)) >= 3 ] = 'control'

import_metadata$diagnosis[ as.numeric(as.character(import_metadata$cogdx)) == 4 & 
                           as.numeric(as.character(import_metadata$braaksc)) >= 4 & 
                           as.numeric(as.character(import_metadata$ceradsc)) <= 2] = 'AD'
...
##_##METADATA <- METADATA[, (colnames(METADATA)%in%'pmi.1')==F]

##_##colnames(METADATA)[ colnames(METADATA) == 'race.1' ] <- 'race'
##_##colnames(METADATA)[ colnames(METADATA) == 'rincontinuous' ] <- 'rin'

##_##METADATA <- as.data.frame( cbind( Sampleid = row.names(METADATA), METADATA[ ,colnames(METADATA) ] ))
  
##_####_##METADATA <- METADATA %>%
##_##  ungroup %>%
##_##  dplyr::filter(Sampleid %in% colnames(COUNT)) %>%
##_##  dplyr::filter(!is.na(cogdx), !is.na(braaksc), !is.na(ceradsc)) %>%
##_##  dplyr::filter(!is.na(rin)) %>%
##_##  dplyr::filter(!is.na(pct_intronic_bases)) %>%
##_##  dplyr::filter(!is.na(pmi)) %>%
##_##  dplyr::filter(!is.na(age_death)) %>%
##_##  as.data.frame()

# Add harmonised case-control status
##_##METADATA$cogdx <- as.numeric(METADATA$cogdx)
##_##METADATA$braaksc <- as.numeric(METADATA$braaksc)
##_##METADATA$ceradsc <- as.numeric(METADATA$ceradsc)

##_##METADATA$Diagnosis = 'OTHER'
##_##METADATA$Diagnosis[METADATA$cogdx == 1 & METADATA$braaksc <= 3 & METADATA$ceradsc >= 3] = 'CONTROL'
##_##METADATA$Diagnosis[METADATA$cogdx == 4 & METADATA$braaksc >= 4 & METADATA$ceradsc <= 2] = 'AD'

##_##METADATA <- METADATA[ , (colnames(METADATA) %in% "diagnosis") == F ]

```
Dorsolateral prefrontal cortex of `r dim(COUNT)[2]` subjects from the ROS and MAP cohorts are used for the analysis. Following sample are removed due to missing metadata 

### Covariate clustering
Determine relationship between covariates
```{r covariates.clustering, cache=FALSE}
METADATA$rin <- METADATA$rincontinuous
...
```{r filter.genes}
##_##genesToAnalyze = COVARIATES %>%
##_##  rownameToFirstColumn('SampleID') %>%
##_##  dlply(.(Tissue.Diagnosis), .fun = function(mtd, count){
##_##    processed.counts = getGeneFilteredGeneExprMatrix(count[,mtd$SampleID],
##_##                                                     MIN_GENE_CPM=1, 
##_##                                                     MIN_SAMPLE_PERCENT_WITH_MIN_GENE_CPM=0.5)
##_##    processed.counts$filteredExprMatrix$genes
##_##  }, COUNT)
#filter out non-variable genes post residual removal

##_##genesToAnalyze = unlist(genesToAnalyze) %>% 
##_##  unique() %>%
##_##  intersect(GENE.GC$ensembl_gene_id[!is.na(GENE.GC$percentage_gene_gc_content)]) %>%
##_##  intersect(GENE.LEN$ensembl_gene_id[!is.na(GENE.LEN$gene_length)]) %>%
##_##  setdiff(c("N_unmapped", "N_multimapping", "N_noFeature", "N_ambiguous")) %>%
##_##  setdiff( c("ENSG00000239467", "ENSG00000005486", "ENSG00000160972", "ENSG00000165886", "ENSG00000002330", "ENSG00000149823", "ENSG00000139722", "ENSG00000169217", "ENSG00000127774", "ENSG00000122490", "ENSG00000130881", "ENSG00000126005", "ENSG00000171222", "ENSG00000070413", "ENSG00000211459") )

#18404
#vs
#18415
##_##PROCESSED_COUNTS = getGeneFilteredGeneExprMatrix(COUNT[genesToAnalyze, ], 
##_##                                                 MIN_GENE_CPM=0, 
##_##                                                 MIN_SAMPLE_PERCENT_WITH_MIN_GENE_CPM=0)

#_#Make Empty Processed Counts object:
PROCESSED_COUNTS <- list()
##_## Ensemble2HGNC <- GENE.PARAM[ GENE.PARAM$ensembl_gene_id %in% PROCESSED_COUNTS$filteredExprMatrix$genes$genes, ] 
Ensemble2HGNC <- GENE.PARAM[ GENE.PARAM$ensembl_gene_id %in% row.names(cqn_counts$counts), ] 
summary(factor(Ensemble2HGNC$gene_biotype)) %>%
  rownameToFirstColumn('Biotype') %>%
  dplyr::rename(fraction = DF) %>%
  dplyr::mutate(fraction = fraction/dim(cqn_counts$counts)[1]) %>%
  dplyr::filter(fraction >= 0.01) %>%
  kable
```
Processing `r dim(cqn_counts$counts)[1]` genes in `r dim(cqn_counts$counts)[2]` samples
...
p
##_##indToRetain = setdiff(colnames(PROCESSED_COUNTS$filteredExprMatrix$counts), indToRemove)
##_##PROCESSED_COUNTS$filteredExprMatrix$counts = PROCESSED_COUNTS$filteredExprMatrix$counts[,indToRetain]
##_##CQN.GENE_EXPRESSION$E = CQN.GENE_EXPRESSION$E[,indToRetain]
##_##COVARIATES = COVARIATES[indToRetain,]
tmp = COVARIATES %>%
  tidyr::separate(Tissue.Diagnosis, c('Tissue', 'Diagnosis'), sep = '\\.') %>%
  dplyr::group_by(Diagnosis, Tissue) %>%
  dplyr::summarise(count = n()) %>%
  tidyr::spread(Diagnosis, count)
PROCESSED_COUNTS$filteredExprMatrix <- cqn_counts$counts
```
...
# Find principal components of expression to plot
##_## Reassign to new
LIB.SIZE = colSums( cqn_counts$counts )
...
DESIGN$design = DESIGN$design[,linColumnFinder(DESIGN$design)$indepCols]
colnames(DESIGN$design) = gsub('Tissue.Diagnosis.SEX','',colnames(DESIGN$design))
# Estimate voom weights for dispersion control
VOOM.WEIGHTS = voom(cnts, design=DESIGN$design, plot=F)
# Fit linear model using new weights and new design
FIT = lmFit(CQN.GENE_EXPRESSION$E,
...
contrast = makeContrasts(contrasts=c("(ACC.AD.female+ ACC.AD.male)/2-( ACC.control.female+ACC.control.male)/2",
                                     "(ACC.AD.female+ ACC.control.female)/2-( ACC.AD.male+ACC.control.male)/2",
                                     "(ACC.AD.female- ACC.control.female)/2-( ACC.AD.male-ACC.control.male)/2",
                                     "ACC.AD.female- ACC.control.female",
                                     "ACC.AD.male- ACC.control.male",
                                     "( DLPFC.AD.female+ DLPFC.AD.male)/2-( DLPFC.control.female+ DLPFC.control.male)/2",
                                     "( DLPFC.AD.female+ DLPFC.control.female)/2-( DLPFC.AD.male+ DLPFC.control.male)/2",
                                     "( DLPFC.AD.female- DLPFC.control.female)/2-( DLPFC.AD.male- DLPFC.control.male)/2",
                                     "DLPFC.AD.female- DLPFC.control.female",
                                     "DLPFC.AD.male- DLPFC.control.male",
                                     "( PCC.AD.female+ PCC.AD.male)/2-( PCC.control.female+ PCC.control.male)/2",
                                     "( PCC.AD.female+ PCC.control.female)/2-( PCC.AD.male+ PCC.control.male)/2",
                                     "( PCC.AD.female- PCC.control.female)/2-( PCC.AD.male- PCC.control.male)/2",
...
names(DE) = c('ACC-AD-CONTROL','ACC-FEMALE-MALE','ACC-AD-CONTROL.IN.FEMALE-MALE','ACC-AD-CONTROL.IN.FEMALE', 'ACC-AD-CONTROL.IN.MALE',
              'DLPFC-AD-CONTROL','DLPFC-FEMALE-MALE','DLPFC-AD-CONTROL.IN.FEMALE-MALE','DLPFC-AD-CONTROL.IN.FEMALE', 'DLPFC-AD-CONTROL.IN.MALE',
              'PCC-AD-CONTROL','PCC-FEMALE-MALE','PCC-AD-CONTROL.IN.FEMALE-MALE','PCC-AD-CONTROL.IN.FEMALE', 'PCC-AD-CONTROL.IN.MALE')
DE = DE %>% 
  rbindlist(idcol = 'Comparison') %>%
  left_join(GENE.PARAM %>%
              dplyr::select(ensembl_gene_id, hgnc_symbol, percentage_gene_gc_content, gene_length) %>%
...
tmp = DE %>%
  dplyr::select(ensembl_gene_id, Region, Comparison, Direction) %>%
...
temp<-colnames(EXP)
```
```{r NeuroPath_Associtation_1_1, include=FALSE, cache=TRUE}
#BRAAK
Path <- 'braaksc'
mark<-Sys.time()
BRAAK_ACC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'ACC' )}, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_1_2, include=FALSE, cache=TRUE}
mark<-Sys.time()
BRAAK_PCC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'PCC' )}, error=function(e) NULL)
Sys.time() - mark
```

```{r NeuroPath_Associtation_1_3, include=FALSE, cache=TRUE}
mark<-Sys.time()
BRAAK_DLPFC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'DLPFC' )}, error=function(e) NULL)
Sys.time() - mark

BRAAK <- rbind( BRAAK_ACC, BRAAK_PCC, BRAAK_DLPFC )
```
```{r NeuroPath_Associtation_2_1, include=FALSE, cache=TRUE}
#CERAD
Path <- 'ceradsc'
mark<-Sys.time()
CERAD_ACC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'ACC' )}, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_2_2, include=FALSE, cache=TRUE}
mark<-Sys.time()
CERAD_PCC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'PCC' )}, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_2_3, include=FALSE, cache=TRUE}

mark<-Sys.time()
CERAD_DLPFC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'DLPFC' )}, error=function(e) NULL)
Sys.time() - mark

CERAD <- rbind( CERAD_ACC, CERAD_PCC, CERAD_DLPFC )
```
```{r NeuroPath_Associtation_3_1, include=FALSE, cache=TRUE}

#COGDX
Path <- 'cogdx'
mark<-Sys.time()
cogdx_ACC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'ACC' )}, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_3_2, include=FALSE, cache=TRUE}

mark<-Sys.time()
cogdx_PCC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'PCC' )}, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_3_3, include=FALSE, cache=TRUE}

mark<-Sys.time()
cogdx_DLPFC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'DLPFC' )}, error=function(e) NULL)
Sys.time() - mark
```

```{r NeuroPath_Associtation_4_1, include=FALSE, cache=TRUE}
...
Path <- 'dcfdx_lv'
mark<-Sys.time()
dcfdx_ACC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'ACC' ) }, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_4_2, include=FALSE, cache=TRUE}

mark<-Sys.time()
dcfdx_PCC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'PCC' ) }, error=function(e) NULL)
Sys.time() - mark
```
```{r NeuroPath_Associtation_4_3, include=FALSE, cache=TRUE}
mark<-Sys.time()
dcfdx_DLPFC <- foreach( i=1:length(colnames(EXP)), .export=c( 'Path', 'EXP', 'temp'), .packages = c('foreign', 'MASS', 'Hmisc'), .combine = 'rbind' ) %dopar% tryCatch({NeuroPath_Calc( GN=temp[i],Path=Path, Exp=EXP, Tiss = 'DLPFC' ) }, error=function(e) NULL)
Sys.time() - mark
...
thisFileName <- '2020_12_09_ROSMAP_FullCohort_sageseqr_hg38.Rmd'
# Github link
...
CODE <- syn_temp$store(synapseclient$Folder(name = "Full_RosMap_sageseqr", parentId = "syn23640229"))

Syns_Used <- c( "syn23643275", "syn23643273",
                "syn23625835", "syn21075913"
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'ROSMAP_Regressed_Counts.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_Counts.tsv', name = 'Counts (filtered raw)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
NEW.COUNTS %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'ROSMAP_Regressed_eCounts.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_eCounts.tsv', name = 'Counts (estimated)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
CQN.GENE_EXPRESSION$y %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'ROSMAP_Regressed_logCPM.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'ROSMAP_Regressed_offset.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_offset.tsv', name = 'Gene length and GC content offset', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
MODEL1 %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'ROSMAP_Regressed_Design.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_Design.tsv', name = 'Design Matrix', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
SURR.VAR %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'ROSMAP_Regressed_SurVar.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_SurVar.tsv', name = 'Surrogate Variables', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
RESIDUAL.NET.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'ROSMAP_Regressed_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_Regressed_netResidualExpression.tsv', name = 'Normalised, and covariates removed residual expression (for network analysis)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
rbindlist(all.diff.exp, use.names = T, fill = T, idcol = 'Model') %>%
  write.table(file = 'ROSMAP_DiffExpression.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='ROSMAP_DiffExpression.tsv', name = 'Differential Expression Results (cogdx)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
```{r knitmd, eval=FALSE, cache=FALSE, include=FALSE}
  ...
  createAndKnitToFolderEntityClient(file = "2020_12_09_ROSMAP_FullCohort_sageseqr_hg38.Rmd",
                                            parentId ="syn23640229",
                                            folderName = 'Full_RosMap_sageseqr')
