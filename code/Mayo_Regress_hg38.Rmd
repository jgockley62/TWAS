---
title: Covariate Analysis of MAYO Reprocessed counts
...
| syn8698211.1 | | 
| syn3817650.8 | | 
| Oldcounts------syn8690904.1 | | 
| syn6126119.3 | | 
| syn20831294 ||
| OldMetrics--------syn8698214.1 | | 
| OldGeneModel------syn8449369.2 | | 
| syn6126114.3 | | 
| syn6145639.5 | | 
| OldCounts------syn8690799.1 | | 
|syn20830935.1 | |
|syn20825567.1 | |
|syn21156161.2 | |
...
#_#OLD:COUNT_TC_ID <- 'syn8690799';
#_#OLD:ALL_USED_IDs <- COUNT_TC_ID
#_#OLD:COUNT_TC <- syn_temp$get(COUNT_TC_ID, version = 1)$path %>%
#_#OLD:  read.table(header=T, sep='\t', check.names = F, row.names = 1)
#_#OLD:# Get reprocessed sample counts for cerebellum
#_#OLD:COUNT_CE_ID <- 'syn8690904';
#_#OLD:ALL_USED_IDs[length(ALL_USED_IDs)+1] <- COUNT_CE_ID
#_#OLD:COUNT_CE <- syn_temp$get(COUNT_CE_ID, version = 1)$path %>%
#_#OLD:  read.table(header=T, sep='\t', check.names = F, row.names = 1)

COUNT_ID <- 'syn20830935';
ALL_USED_IDs <- 'syn20830935'
...
#_#OLD:COUNT = cbind(COUNT_TC, COUNT_CE[rownames(COUNT_TC),])
...
METADATA_TC <- syn_temp$get(METADATA_TC_ID, version = 8)$path %>%
...
#METADATA_PICARD_TC <- syn_temp$get(METADATA_PICARD_TC_ID, version = 1)$path %>%
#  read.table(sep='\t', header=T, stringsAsFactors=F) %>% 
#  dplyr::rename(ID = sample)
...
METADATA.PICARD <- 'syn20831294';
ALL_USED_IDs[length(ALL_USED_IDs)+1] = METADATA.PICARD
METADATA.PICARD <- syn_temp$get(METADATA.PICARD, version = 1)$path %>%
  read.table(sep='\t', header=T, stringsAsFactors=F) %>% 
  dplyr::rename(ID = sample)

#REplace Syn IDs
row.names(METADATA.PICARD) <- as.character( Translator[ METADATA.PICARD$ID , ][,2] )
METADATA.PICARD$ID <- row.names(METADATA.PICARD)

METADATA = list(METADATA_TC, METADATA_CE) %>%
  data.table::rbindlist(use.names = T, fill = T) %>%
  dplyr::inner_join(list(METADATA.PICARD) %>% 
                      data.table::rbindlist(use.names = T, fill = T)) %>%
  as.data.frame
...
SAMPLES.EXCLUDE = c(read.table(syn_temp$get('syn6126119', version = 3)$path, sep = '\t', header = T, stringsAsFactors = F)$Sample.Name,
                    read.table(syn_temp$get('syn6126114', version = 3)$path, sep = '\t', header = T, stringsAsFactors = F)$Sample.Name)
...
CONTROL.PROBES = fread(syn_temp$get(CONTROL.PROBES_ID, version = 5)$path, data.table = F) %>%
  dplyr::mutate(ID = x) %>%
  tidyr::separate(ID, c('ensembl_gene_id', 'position'), sep = '\\.')
...
thisFileName <- 'Mayo_Regress_hg38.Rmd'
# Github link
...
activityName = 'Covariate Regression';
activityDescription = 'Covariate analysis of aligned effective counts with GRCh38 with CQN normalisation (CBE, TCX)';

CODE <- syn_temp$store(synapseclient$Folder(name = "Mayo", parentId = parentId))

#Set Used SynIDs For Provenance
Syns_Used <- c("syn5223705", "syn8698211", "syn3817650", "syn20825567",
"syn6126119", "syn20831294", "syn8449369", "syn6126114", "syn6145639", "syn20830935"
)
...
# Store SVA results
write.table(RESIDUAL.SVA.GENE_EXPRESSION, file = 'MAYO_SVA_Corrected.tsv', sep = '\t', quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_SVA_Corrected.tsv', name = 'Normalised, covariates, and surrogate variable adjusted residual expression (for eQTL)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
write.table(COVARIATES, file = 'MAYO_SVA_Covariates.tsv', row.names = F, sep = '\t', quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_SVA_Covariates.tsv', name = 'Covariates', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
PROCESSED_COUNTS$filteredExprMatrix$counts %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'MAYO_Regressed_Counts.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_Counts.tsv', name = 'Counts (filtered raw)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
NEW.COUNTS %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'MAYO_Regressed_eCounts.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_Counts.tsv', name = 'Counts (estimated)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
CQN.GENE_EXPRESSION$y %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'MAYO_Regressed_logCPM.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_logCPM.tsv', name = 'Counts (filtered logCPM)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
CQN.GENE_EXPRESSION$offset %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'MAYO_Regressed_offset.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_offset.tsv', name = 'Gene length and GC content offset', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
MODEL1 %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'MAYO_Regressed_Design.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_Design.tsv', name = 'Design Matrix', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
SURR.VAR %>%
  rownameToFirstColumn('SampleID') %>%
  write.table(file = 'MAYO_Regressed_SurVar.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_SurVar.tsv', name = 'Surrogate Variables', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
RESIDUAL.NET.GENE_EXPRESSION %>%
  rownameToFirstColumn('ensembl_gene_id') %>%
  write.table(file = 'MAYO_Regressed_netResidualExpression.tsv', sep = '\t', row.names=F, quote=F)
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_Regressed_netResidualExpression.tsv', name = 'Normalised, and covariates removed residual expression (for network analysis)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_CBE_TCX_DiffExpression.tsv', name = 'Differential Expression Results (Tissue.Diagnosis)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
ENRICH_OBJ <-  syn_temp$store( synapseclient$File( path='MAYO_CBE_TCX_allfits.RData', name = 'All model fits (Tissue.Diagnosis)', parentId=CODE$properties$id ), used = Syns_Used, activityName = activityName, executed = thisFile, activityDescription = activityDescription)
...
createAndKnitToFolderEntityClient(file = "Mayo_Regress_hg38.Rmd",
                                          parentId ="syn21277617",
                                          folderName = 'Mayo')
