...
# Elastic Net #_# Need to re-align the CV.glmnet to cross validate correctly
weights.enet = function( Genos , Pheno , alpha=0.5, scale=opt$scale ) {
	#_# Genos <- genos$bed[ paste0( cv.train[,1], ':', cv.train[,2] ),] 
	#_# Pheno <- as.matrix(cv.train[,3,drop=F]) 
	#_# beta=T
	eff.wgt = matrix( 0 , ncol=1 , nrow=ncol(Genos) )
	# remove monomorphics
	sds = apply( Genos  , 2 , sd )
	keep = sds != 0 & !is.na(sds)
	#Build foldid vecotr specifying which fold each ID is in, if data is scaled.
	if( opt$scale > 0 ){
		#Isolate fid and iid
		fid <- do.call(rbind, strsplit( row.names(Genos), ':' ) )[,2]
		iid <- do.call(rbind, strsplit( row.names(Genos), ':' ) )[,1]
		cv.all <- as.data.frame(cbind(V1=iid,V2=fid,V3=Pheno))

		tFID <- fid[ !duplicated(fid) ]
		
		#Sample unique FIDs into 5 CV groups
		folds = cut(seq(1,length(tFID)),breaks=opt$crossval,labels=FALSE)
		names(folds) <-  sample(tFID)
		#table(folds[as.character(cv.all$V2)])
		cv.all = cv.all[ order(match(cv.all$V2, names(folds))), ] 
		folds <- folds[as.character(cv.all$V2)]
		HET <- min(table(folds))/max(table(folds))

		##Keep permuting CV fold designation untill all groups are within 95% of their size
		while( HET < .95 ){
			#cv.all = as.data.frame(cbind(V1=iid,V2=fid))
			folds = cut(seq(1,length(tFID)),breaks=opt$crossval,labels=FALSE)
			names(folds) <-  sample(tFID)
			#table(folds[as.character(cv.all$V2)])
			cv.all = cv.all[ order(match(cv.all$V2, names(folds))), ] 
		...
		Names <- paste0( as.character(cv.all$V1) , ':', as.character(cv.all$V2) )
		enet = cv.glmnet( x=Genos[Names,keep] , y=as.numeric(as.character(cv.all[,3])) , foldid=as.numeric(folds[as.character(cv.all$V2)]) , alpha=alpha , intercept=T , standardize=F )
		#_#enet = cv.glmnet( x=Genos[Names,keep] , y=as.numeric(as.character(cv.all[,3])) , nfold=5 , alpha=alpha , intercept=T , standardize=F )
	}else{
...
if ( !is.na(opt$covar) ) {
	...
	#_# Scaled in Original
	if( opt$scale > 0 ){
		message("WARNING: Scaled Data input is not tested with covariate specification")
		pheno[,3] = reg$resid
	}else{
...
#_#SCALE Pheno
if( opt$scale > 0 ){
	message("Treating as Pre-Scaled Data")
	pheno[,3] = pheno[,3]
}else{
...
if ( !is.na(opt$covar) && opt$resid ) {
	...
	#_#Scale Genotypes?
	message("WARNING if scaled data is input, this is not tested to handle covariates and scaled data")
	genos$bed = scale(genos$bed)
}
...
##Sampling occurs here VVV
#_# N = nrow(cv.all) - Samples by Row Numbers
N = cv.all$V2[ !duplicated(cv.all$V2) ] # - Samples by Family IDs
folds = cut(seq(1,length(N)),breaks=opt$crossval,labels=FALSE)
cv.sample <- sample(N)
names(folds) <- cv.sample
cv.all = cv.all[order(match(cv.all$V2, cv.sample)),] 
folds <- folds[cv.all$V2]
HET <- min(table(folds))/max(table(folds))
...
#_#cv.calls = matrix(NA,nrow=N,ncol=M)
cv.calls = matrix(NA,nrow=length(folds),ncol=M)
row.names(cv.calls) <- paste0( cv.all$V1, ":", cv.all$V2 )
...
for ( i in 1:opt$crossval ) {
	...
	for ( mod in 1:M ) {
		if ( models[mod] == "blup" ) {
			...
			#_#pred.wgt = weights.enet( genos$bed[ cv.sample[ -indx ],] , as.matrix(cv.train[,3]) , alpha=0.5 )
			pred.wgt = weights.enet( genos$bed[ paste0( cv.train[,1], ':', cv.train[,2] ),] , as.matrix(cv.train[,3]) , alpha=0.5 )
		}		
		else if ( models[mod] == "top1" ) {
			#_# pred.wgt = weights.marginal( genos$bed[ cv.sample[ -indx ],] , as.matrix(cv.train[,3,drop=F]) , beta=T )
			pred.wgt = weights.marginal( genos$bed[ paste0( cv.train[,1], ':', cv.train[,2] ),] , as.matrix(cv.train[,3,drop=F]) , beta=T )
			pred.wgt[ - which.max( pred.wgt^2 ) ] = 0
		}

		# predict from weights into sample
		pred.wgt[ is.na(pred.wgt) ] = 0
		#_#cv.calls[ indx , mod ] = genos$bed[ cv.sample[ indx ] , ] %*% pred.wgt
		cv.calls[ paste0( cv.train[,1], ':', cv.train[,2]) , mod ] = genos$bed[ paste0( cv.train[,1], ':', cv.train[,2] ), ] %*% pred.wgt
	}
}
...
#_# Are the rows of  genos$bed  and  pheno[,3] the same? :: ANSWER: YES, All good #_#
