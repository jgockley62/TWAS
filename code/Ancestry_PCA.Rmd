...
```{bash pull_1000Genomes, echo=T, results='hide', cache = FALSE, echo=FALSE}
source /root/.bashrc
...
```{python ReNamer, echo=T, results='hide'}
import sys
import os
import re
...
if [ "$Repod" -eq "0" ]; then
   mv GWASTotalRenamed.bim GWASTotal.bim;
fi
if [ "$Repod" -ne "0" ]; then
...
```{python ReNamerMayo, echo=T, results='hide'}
import sys
import os
import re
...
### ReName the Mayo SNPS so the can be combined
```{python ReNamerRosMap, echo=T, results='hide'}
import sys
import os
import re
...
```{r Store, cache = FALSE, echo=FALSE, results='hide'}
parentId = 'syn20798011';
activityName = 'Ancestry Analysis';
activityDescription = 'Processing and Analysis of WGS and Genotype data along with ancestry analysis with 1000 genome\'s CEU, CHB, YRI, and JPT populations';
...
# get the package from devtools::install_github('Sage-Bionetworks/knit2synapse')
synLogin()
