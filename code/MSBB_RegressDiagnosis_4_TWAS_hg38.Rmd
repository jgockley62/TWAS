...
while(length(residualSigCovars$significantCovars)!=0 && loopCount <= 20){ #_# <= 8
  ...
  #if( residualSigCovars$significantCovars == 'PCT_RIBOSOMAL_BASES' & isTRUE(length(residualSigCovars$significantCovars) == 1)){
   # foo <- foo + 1 
  #}
  #if( foo > 2){
   # loopCount <- 22
...
activityDescription = 'Covariate analysis and Regrsison of AD Diagnosis of aligned effective counts with GRCh38 with CQN normalisation (IFG, STG, FP, PHG)';
thisFileName <- 'MSBB_RegressDiagnosis_4_TWAS_hg38.Rmd'
# Github link
...
createAndKnitToFolderEntityClient(file = "MSBB_RegressDiagnosis_4_TWAS_hg38.Rmd",
                                          parentId ="syn21136908",
                                          folderName = 'MSBB')
```