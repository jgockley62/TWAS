...
```{bash PLINKSetup, echo=F, include = T, results='hide', eval=T }
mkdir ~/TWAS/bin
...
wget -q -P ~/TWAS/bin  http://s3.amazonaws.com/plink2-assets/plink2_linux_x86_64_20191002.zip
unzip -d ~/TWAS/bin/ ~/TWAS/bin/plink2_linux_x86_64_20191002.zip
...
Process_MSBB <- function( dna, exp, id){
  ...
  Expression <- read.table(syn_temp$get(exp)$path, header=T)
...
```{bash FilterGenos, include=FALSE, cache=TRUE}
#source /root/.bashrc
PATH=$PATH:~/TWAS/bin/
CORES=`expr $(nproc) - 2`
...
```{r synapse.parameters.head, include=FALSE, cache=TRUE}
...
## TOTAL SVA-only Exp
write.table(Norm_Resid, file="ALL_RNA_Seq_SVAadusted.tsv", col.names =F, row.names=F, sep="\t", quote=F)
...
## CEU Diagnosis Exp
write.table(Diag_Resid_train, file="ALL_CEU_RNA_Seq_SVAadusted_DiagnosisRegressed.tsv", col.names =F, row.names=F, sep="\t", quote=F)
Syns_Used_Diagnosis_EXP <- c('syn20801991', 'syn11384608', 'syn20801798', 'syn6100548',  'syn20800652', 'syn5879839', 'syn3382527','syn20820122', 'syn20820122')
...
write.table(ROS_Norm_Resid_train, file="ROS_CEU_RNA_Seq_SVAadusted.tsv", col.names =F, row.names=F, sep="\t", quote=F)
...
write.table(ROS_Diag_Resid_train, file="ROS_CEU_RNA_Seq_SVAadusted_DiagnosisRegressed.tsv", col.names =F, row.names=F, sep="\t", quote=F)
